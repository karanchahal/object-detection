...
def get_indices(sentence):
    i = tokenizer.tokenize(sentence)
    indices = []
    for j in i:
        indices.append(vocab.idx(j))
    indices.append(vocab.idx('<EOS>'))
    return indices
...
lines = open('./ayo.txt', encoding='utf-8').\
    read().strip().split('\n')
...
while True:
    test = input()
    print(test)
    output_words, attentions = evaluate(encoder1, attn_decoder1, get_indices(  trim(normalizeString(test.strip())) ))
    output_sentence = ' '.join(output_words)
    print('<', output_sentence)
